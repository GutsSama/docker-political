...
git pull origin vps-deployment --rebase
...
# 5. Redémarrer les services en zéro-downtime (recréation des conteneurs uniquement si l'image a changé)
...
sleep 5
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost/home || echo "000")
