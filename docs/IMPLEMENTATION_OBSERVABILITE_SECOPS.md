...
- Il est monté avec des accès en lecture seule (`:ro`) au système de fichiers hôte (`/proc`, `/sys`) pour récolter les informations du VPS physique.
...
- Il écoute directement le démon Docker (`/var/lib/docker`) pour analyser combien de RAM et de CPU consomment spécifiquement Django, FastAPI, Postgres, etc.
...
Ce sprint met en place la stack de log pour éviter de devoir se connecter en SSH pour lire les erreurs des conteneurs.

### 1. Loki

**Branche :** `feature/loki-logging`

**Technologie :** Le système de stockage et requête de logs développé par Grafana, conçu pour être économe et s'intégrer parfaitement avec Prometheus.

**Implémentation :**

- Ajouté dans `docker-compose.monitoring.yml` et configuré via `monitoring/loki/loki-config.yaml`.
- Loki n'indexe pas le texte complet des logs (ce qui serait lourd), mais uniquement leurs métadonnées (labels), ce qui permet de stocker des logs massifs avec un minimum d'empreinte mémoire sur notre VPS de 8Go.

### 2. Promtail

**Branche :** `feature/loki-logging` (inclus)

**Technologie :** L'agent de collecte de logs qui alimente Loki.

**Implémentation :**

- Ajouté dans `docker-compose.monitoring.yml` et configuré via `monitoring/promtail/promtail-config.yaml`.
- Il est monté en lecture seule sur les dossiers de logs locaux du serveur : `/var/log` (système) et `/var/lib/docker/containers` (logs des conteneurs).
- Il lit ces fichiers JSON en direct, les parse, leur ajoute des labels utiles, puis les envoie en Push à Loki via l'API HTTP.
